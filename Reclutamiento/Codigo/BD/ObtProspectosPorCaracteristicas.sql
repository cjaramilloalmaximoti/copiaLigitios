...
DELIMITER $$
USE `directma_reclutamiento`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `ObtProspectosPorCaracteristicas`(IN `pIdEmpresa` INT, IN `ParametrosXML` TEXT, IN `pIdSexo` INT,IN `pIdEstadoCivil` INT,IN `pIdEscolaridad` INT,IN `pEdadMin` INT,IN `pEdadMax` INT, IN `pIdVacante` INT)
BEGIN
