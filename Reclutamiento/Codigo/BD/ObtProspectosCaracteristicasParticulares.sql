...
DELIMITER $$
USE `directma_reclutamiento`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `ObtProspectosCaracteristicasParticulares`(
	IN `pNombre` VARCHAR(255), 
	IN `pActivo` INT, 
	IN `pIdCaracteristica` VARCHAR(5000),  
	IN `pValor` VARCHAR(5000), 
	IN `pIdEmpresa` INT,
    IN `pCaracteristicasGenerales`varchar(5000))
BEGIN
