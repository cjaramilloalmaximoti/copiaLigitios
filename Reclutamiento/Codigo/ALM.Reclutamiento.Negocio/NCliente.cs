...
namespace ALM.Reclutamiento.Negocio
{
    public class NCliente
    {
        ...
        {
            if(razonSocial != null  && razonSocial.Length >64)
            {
                throw new Exception("La longitud máxima de la razón social es de 64 caracteres");
            }
        ...
        {
            return new DCliente().ObtenerClienteId(IdCliente);
        }

        public List<EDocumentos> ObtenerDocumentos(int activo, int idEmpresa)
        {
            return new DCliente().ObtenerDocumentos(activo, idEmpresa);
        }
